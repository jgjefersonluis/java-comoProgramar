...
class GuiPrincipal extends JFrame {
	...
	private void setConfig() {
		...
		this.setLayout(new GridLayout(0, 2));// layout se comporta como uma
												// tabela, mais detalhes:
												// http://docs.oracle.com/javase/tutorial/uiswing/layout/grid.html
		this.setDefaultCloseOperation(EXIT_ON_CLOSE);// fecha aplicação como
														// padrão
		this.setLocationRelativeTo(null);// centraliza o JFrame
		this.getContentPane().setBackground(Color.YELLOW);// cor de fundo é
															// amarelo
	...
	class EventoCalculaIMC implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro aritmético, causa: "
						+ ar.getMessage());
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(null,
						"Erro desconhecido, causa: " + ex.getMessage());
			}
		}
	}
